...
# ------------------------------------------------------------------
st.header("4 · Consultant Percentages (% of Construction Cost)")
consultant_pct = {
...
st.download_button("📥 Download Fee Summary CSV", csv_summary, "fee_summary.csv", "text/csv")
st.download_button("📥 Download Schedule CSV", csv_schedule, "project_schedule.csv", "text/csv")

# ------------------------------------------------------------------
# 7. Build Gantt Chart
# ------------------------------------------------------------------
fig_timeline = px.timeline(df_schedule, x_start="Start", x_end="End", y="Phase")
fig_timeline.update_yaxes(autorange="reversed")

# ------------------------------------------------------------------
# 8. PDF Report
# ------------------------------------------------------------------
st.header("7 · Download PDF Report")
def build_pdf() -> BytesIO:
    ...
    # Schedule table & timeline image on new page
    pdf.add_page()
    pdf.set_font("Arial", "B", 12)
    ...
    # Save Gantt chart as image using kaleido
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmpfile:
        fig_timeline.write_image(tmpfile.name, width=800, height=400)
        chart_path = tmpfile.name
    pdf.add_page()
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 8, "Schedule Gantt Chart", ln=True)
...
st.download_button("📄 Download PDF Report", data=build_pdf(),
